...
function wp_50_slider() 
{
	...
	?>
    <!-- begin wp_50_photo -->
    <script type="text/javascript" src="<?php echo get_option('siteurl'); ?>/wp-content/plugins/wp-photo-text-slider-50/js/jquery-1.3.2.min.js"></script>
    <script type="text/javascript" src="<?php echo get_option('siteurl'); ?>/wp-content/plugins/wp-photo-text-slider-50/js/jquery.cycle.all.min.js"></script>
    <link rel="stylesheet" type="text/css" href="<?php echo get_option('siteurl'); ?>/wp-content/plugins/wp-photo-text-slider-50/wp-photo-text-slider-50.css" />
    <div id="wp_50_photo">
    <?php
...
function wp_50_install() 
{
	...
	if($wpdb->get_var("show tables like '". WP_PHOTO_50_TABLE . "'") != WP_PHOTO_50_TABLE) 
	{
		...
		for($i=1; $i<6; $i++)
		{
			$sSql = $IsSql . " VALUES ('http://www.gopiplus.com/work/wp-content/uploads/haw-pav-villa/200x150/sing_$i.jpg', 'http://www.gopiplus.com/work/', '_blank', '$title', '$i', 'YES', 'gallery1', 'Wp photo slider $i', '0000-00-00 00:00:00');";
			$wpdb->query($sSql);
			$sSql = "";
		}
	}
...
function wp_50_admin_options() 
{
	echo "<div class='wrap'>";
	echo "<h2>"; 
	echo wp_specialchars( "Wp photo text slider 50" ) ;
	echo "</h2>";
...
function wp_50_show_filter_callback($matches) 
{
    ...
    $wp_50 = $wp_50 .'<script type="text/javascript" src="'.$wp_50_pluginurl.'/js/jquery-1.3.2.min.js"></script>';
    $wp_50 = $wp_50 .'<script type="text/javascript" src="'.$wp_50_pluginurl.'/js/jquery.cycle.all.min.js"></script>';
    $wp_50 = $wp_50 .'<link rel="stylesheet" type="text/css" href="'.$wp_50_pluginurl.'/wp-photo-text-slider-50.css" />';
...
{
	echo '<p>Wp photo text slider 50<br><br> To change the setting goto "Wp photo text slider 50" link under setting menu. ';
	echo '<a href="options-general.php?page=wp-photo-text-slider-50/wp-photo-text-slider-50.php">click here</a></p>';
	echo '<a target="_blank" href="http://www.gopiplus.com/work/2011/06/02/wordpress-plugin-wp-photo-slider-50/">Click here</a> for more help.<br>';
}
...
{
  	register_sidebar_widget(__('Wp photo text slider 50'), 'wp_50_widget');   
	
	if(function_exists('register_sidebar_widget')) 	
	{
		register_sidebar_widget('Wp photo text slider 50', 'wp_50_widget');
	}
	
	if(function_exists('register_widget_control')) 	
	{
		register_widget_control(array('Wp photo text slider 50', 'widgets'), 'wp_50_control');
	} 
}
