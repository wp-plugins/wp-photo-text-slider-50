...
if (isset($_POST['wp_50_form_submit']) && $_POST['wp_50_form_submit'] == 'yes')
{
	...
	{	
		$sSql = $wpdb->prepare(
				...
				WHERE wp_50_id = %d
				LIMIT 1",
				array($form['wp_50_path'], $form['wp_50_link'], $form['wp_50_target'], $form['wp_50_title'], $form['wp_50_order'], $form['wp_50_status'], $form['wp_50_type'], $form['wp_50_extra1'], $did)
			);
		$wpdb->query($sSql);
		
		$wp_50_success = 'Image details was successfully updated.';
	}
}
...
if ($wp_50_error_found == FALSE && strlen($wp_50_success) > 0)
{
	...
	<div class="updated fade">
		<p><strong><?php echo $wp_50_success; ?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/admin.php?page=wp-photo-text-slider-50">Click here</a> to view the details</strong></p>
	</div>
	<?php
}
?>
...
<div class="form-wrap">
	...
	<form name="wp_50_form" method="post" action="#" onsubmit="return wp_50_submit()"  >
	  ...
	  <label for="tag-title">Enter image path (URL)</label>
		...
		<label for="tag-title">Select gallery type (This is to group the content)</label>
      ...
      <input name="wp_50_id" id="wp_50_id" type="hidden" value="<?php echo $form['wp_50_id']; ?>">
